...
def forwards_my_app_image(apps, schema_editor):
    try:
        MyAppImage = apps.get_model('my_app', 'Image')
        my_app_image = True
    except LookupError:
        my_app_image = False
    if my_app_image:
        MyAppPicture = apps.get_model('my_app', 'Picture')
        for old_object in MyAppImage.objects.all():
            old_cmsplugin_ptr = old_object.cmsplugin_ptr
            attributes = {}
            if old_object.alt_text:
                attributes.update({'alt': old_object.alt_text})
            new_object = MyAppPicture(
                enable_zoom=old_object.enable_zoom,
                caption_text=old_object.caption_text,
                external_picture=old_object.image_url
                if old_object.image_url else '',
                use_automatic_scaling=old_object.use_autoscale,
...
def forwards(apps, schema_editor):
    ...
    my_app_image = forwards_my_app_image(apps, schema_editor)
    ...
    if not cmsplugin_filer_file and not cmsplugin_filer_folder and not \
       cmsplugin_filer_image and not my_app_image:
        return
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('my_app', '00XY_my_app_previous_migration'),
